...
function Seal({ size = 96, active = false, tone = 'accent', stamp = false }: { size?: number; active?: boolean; tone?: 'accent' | 'invert'; stamp?: boolean }) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 100 100"
      className={`ll-seal ll-seal--${tone} ${stamp ? '' : active ? 'll-seal--active' : ''} ${stamp && active ? 'stamp-impact' : ''}`}
      aria-hidden="true"
    >
...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => setStamped(true), 520);
  ...
  return (
    <div className="overflow-hidden" style={{ background: 'var(--color-bg)' }}>
      ...
      <section className="relative" style={{ paddingTop: 'var(--spacing-12)', paddingBottom: 'var(--spacing-12)' }}>
        <div className="ll-hero-lines" aria-hidden="true" />
        ...
        <div className="container-wide relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 items-center" style={{ gap: 'var(--spacing-8)' }}>
            {/* Left: copy */}
            <div>
              <p className={`type-eyebrow ll-fade-slot ${heroVisible ? 'll-in' : ''}`} style={{ color: 'var(--color-accent)', marginBottom: 'var(--spacing-2)', transitionDelay: '0ms' }}>
                Soroban Smart Contract · Stellar Network
              </p>

              <h1 className={`type-display-2xl ll-fade-slot ${heroVisible ? 'll-in' : ''}`} style={{ color: 'var(--color-ink)', maxWidth: '14ch', marginBottom: 'var(--spacing-3)', transitionDelay: '80ms' }}>
                Escrow,{' '}
                <span className="ll-hero-sealed">sealed</span>{' '}
                on-chain.
              </h1>
            ...
            {/* Right: escrow receipt artifact — dark vault plaque, sealed by a stamping animation */}
            <div ref={receipt.ref} className={`flex justify-center ll-receipt-stage ${receipt.visible ? 'll-receipt-in' : ''}`} aria-hidden="true">
              <div className="ll-receipt">
                ...
                <div className="ll-receipt-seal">
                  ...
                  <Seal size={72} active={receipt.visible && stamped} tone="accent" stamp />
                </div>
              </div>
            </div>
          </div>
      ...
      <section style={{ paddingTop: 'var(--spacing-12)', paddingBottom: 'var(--spacing-12)' }} id="features">
        <div className="container-wide">
          <div className="text-center" style={{ maxWidth: 640, margin: '0 auto var(--spacing-6)' }}>
            <p className="type-eyebrow" style={{ color: 'var(--color-accent)', marginBottom: 12, justifyContent: 'center' }}>Register of Capabilities</p>
      ...
      {/* ── CTA Band ─────────────────────────────────────────────────────────── */}
      <motion.section
        className="ll-cta vault-grain"
        initial={{ opacity: 0, y: 24 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, amount: 0.4 }}
        transition={{ duration: 0.6, ease: [0.2, 0.8, 0.2, 1] }}
      >
        <div className="ll-cta-seal-watermark" aria-hidden="true"><Seal size={320} tone="invert" active /></div>
        <div className="container-wide relative z-10">
          <div className="ll-cta-inner">
            ...
            <p className="type-body" style={{ color: 'rgba(243,238,226,0.62)', maxWidth: '48ch', marginBottom: 'var(--spacing-4)' }}>
      ...
      <style jsx>{`
        .ll-hero-lines {
          ...
          background-image: repeating-linear-gradient(
            180deg,
            rgba(10,11,13,0.035) 0px,
            rgba(10,11,13,0.035) 1px,
            transparent 1px,
            transparent 40px
          );
        ...
        .ll-hero-glow {
          ...
          background: radial-gradient(ellipse at center, rgba(166,116,15,0.10), transparent 70%);
        }
        ...
        .ll-ledger-item { display: grid; grid-template-columns: 44px 40px 1fr; gap: 20px; align-items: start; padding: 26px 4px; border-bottom: 1px solid var(--color-border); transition: background 0.2s ease; }
        .ll-ledger-item:hover { background: var(--color-surface); }
        .ll-ledger-num { color: var(--color-ink-faint); padding-top: 3px; transition: color 0.2s ease; }
        .ll-ledger-item:hover .ll-ledger-num { color: var(--color-accent); }
        ...
        .ll-cta { position: relative; padding: var(--spacing-12) 0; overflow: hidden; background: var(--color-invert-bg); }
        .ll-cta-seal-watermark { position: absolute; right: -60px; top: 50%; transform: translateY(-50%); opacity: 0.07; }
